...
### Changed
- `Emotiva::forms()` and `EmotivaHeart::get_forms()` now take `&self` instead of `&mut self`.
- Transform resolution no longer requires mutable access to tween state.
- This allows Emotiva to integrate with rendering APIs that use `draw(&self)`.

## [0.1.1] - 2026-03-06

### Fixes
- Fix incorrect return value when checking motion/rotation state.

### Improvements
- Add layer name field to `EmotivaForm`.

### Documentation
- Standardize module and example documentation formatting for docs.rs.
